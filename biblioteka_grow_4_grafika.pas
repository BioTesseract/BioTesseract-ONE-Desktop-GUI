...
uses
  GLUT, Classes, SysUtils, LCLIntf, FileUtil, Graphics, Controls, Forms,
  Dialogs, Math, Menus, ExtCtrls, GL, GLU, Buttons,
  LResources, Clipbrd, gettext, Printers, TAGraph, TASeries,
  LCLtype, OpenGLContext, IntfGraphics, fpImage, GLExt, General_dll;

//-- REWRITED

// moved to > graphics.dll -> Function  Main_3Ddraw():boolean;
// moved to > graphics.dll ->  Function  Draw3dAtoms():boolean; //visible,x,y,z,atrybuty [kolor,atr1,atr2]
...
//Ryswanie
  ...
  Function  Draw3dAxes():boolean;    // funkcja wyswitlania os
...
Start3dDraw:boolean=false; // wlacznik / wylacznik rysowania
    ...
    g4g_rendering_quality:string='Low'; // Best lub Low
    ...
    g4g_LookZ:currency=-1; // !!!!!!!!!!!         // pocztakowe polozenie look at na osi Z
    g4g_scale:currency=10; // <--------------------------------------------------------- // saklaownaie co ile
...
begin
    ...
    begin
 {
    Materials(Settings.ComboBox2.ItemIndex); // materialy dla calej sceny
     ...
     glPushMatrix();
        glTranslatef(1,2,3); //Z

        glColor4ub(Red(StringToColor('clRed')),   // Kolory atomow
                   Green(StringToColor('clRed')), // glColor4ub(red, green, blue, alpha)
                   Blue(StringToColor(('clRed'))),g4g_AN1_color_alpha);
 ...
 {     // << gdzies sie pierdolnolem bo nie widac krawedzi !!!!!
 ...
 for Row:=0 to Length(Selected)-1 do
   begin
     glPushMatrix();
        ...
        //glSphere((StrToFloat(Selected[Row,NrColRadius])+0.15) * strtofloat(settings.LabeledEdit1.Text), 10, 10);
...
//Test Planes
 ...
 if Length(Selected) = 3 then
   ...
   glBegin(GL_TRIANGLES);
       glColor4f(Red(StringToColor('clred')),
                 Green(StringToColor('clred')),
                 Blue(StringToColor('clred')),
                 StrToCurr('100')/100); // color

        glVertex3f(strtocurr(Selected[0,NrColX]),strtocurr(Selected[0,NrColY]),strtocurr(Selected[0,NrColZ]));
        glVertex3f(strtocurr(Selected[1,NrColX]),strtocurr(Selected[1,NrColY]),strtocurr(Selected[1,NrColZ]));
        glVertex3f(strtocurr(Selected[2,NrColX]),strtocurr(Selected[2,NrColY]),strtocurr(Selected[2,NrColZ]));
  ...
  { // plane 2
     glPushMatrix();
     glBegin(GL_TRIANGLES);
       glColor4f(Red(StringToColor('clgreen')),
                 Green(StringToColor('clgreen')),
                 Blue(StringToColor('clgreen')),
                 StrToCurr('100')/100); // color


        glVertex3f(strtocurr(Selected[1,NrColX]),strtocurr(Selected[1,NrColY]),strtocurr(Selected[1,NrColZ]));
        glVertex3f(strtocurr(Selected[2,NrColX]),strtocurr(Selected[2,NrColY]),strtocurr(Selected[2,NrColZ]));
        glVertex3f(strtocurr(Selected[3,NrColX]),strtocurr(Selected[3,NrColY]),strtocurr(Selected[3,NrColZ]));
...
glPushMatrix();
  ...
  for i := 0 to Length(text) do
    begin
      //gluStrokeCharacter(GLUT_STROKE_ROMAN, smallint(text[i])); // GLUT_STROKE_ROMAN
    end;
  glPopMatrix();
